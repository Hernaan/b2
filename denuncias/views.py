...
from denuncias.forms import DenunciaForm, RegistroForm
from django.contrib.auth.models import User
#from denuncia.paginacion import Paginate
...
class listar(ListView):
	model = Denuncia
	template_name = 'listar_denuncias.html'
	paginate_by = 10
	ordering = ['-id'] #Si no ordenamos como se muestra los datos no va a funcionar la notificacion

	@method_decorator(permission_required('denuncias.add_denuncia',reverse_lazy('denuncias:pgprincipal')))
...
class DenunciaCreate(SuccessMessageMixin,CreateView):
	...
	template_name = 'denuncia_crear.html'
